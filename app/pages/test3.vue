...
async function loadData() {
  try {
    ...
    const themes = [...new Set(csvData.map(row => row[CSV_CONFIG.columns.theme]))]
    console.log('Unique themes:', themes)
    debugInfo.value = `Found ${themes.length} themes and ${csvData.length} events`
    
    const sankeyData = transformCSVToSankey(csvData, themes)
    console.log('Transformed Sankey data:', sankeyData)
    debugInfo.value = `CSV processed: ${sankeyData.nodes.length} nodes (${themes.length} themes + ${sankeyData.links.length} events)`
...
function transformCSVToSankey(csvData, themes) {
  ...
  const links = sortedData.map((row, index) => {
    const themeIndex = themes.indexOf(row[CSV_CONFIG.columns.theme])
    ...
    return {
      ...
      year: parseInt(row[CSV_CONFIG.columns.year]),
      theme: row[CSV_CONFIG.columns.theme]
    }
  })
...
function createLinks(svg, result, tooltip) {
  return svg.append("g")
    ...
    .attr("fill", "none")
    .attr("opacity", CHART_CONFIG.linkOpacity)
    .attr("class", "link")
...
function addTooltipEvents(selection, tooltip) {
  selection
    ...
    .on("mouseleave", function() {
      d3.select(this).style("opacity", CHART_CONFIG.linkOpacity)
...
function createTitle(svg, width) {
  ...
  svg.append("text")
    ...
    .text("Eduvia Educational History: Themes to Individual Events (1900-1955)")
}
...
function highlightThemeFlows(themeIndex, links, nodes, labels, width) {
  links.style("opacity", CHART_CONFIG.nodeOpacityDimmed)
  ...
  nodes.filter(node => node.x0 < width / 2 && node.index !== themeIndex)
    .style("opacity", CHART_CONFIG.nodeOpacityDimmed)
  
  nodes.filter(node => node.x0 >= width / 2)
    .style("opacity", CHART_CONFIG.linkOpacity)
...
function resetHighlighting(links, nodes, labels) {
  links.style("opacity", CHART_CONFIG.linkOpacity)
